...
""" Class to edit a user's last name.
"""
...
class TestUserLastNameEdited(BaseTest):
    ''' Navigate to the Users page and edit a user's last name. '''

    def test_that_user_last_name_is_edited(self):
        ''' Verify that a user's last name is successfully edited.'''
        ...
        # Wait for user edit form to load, change last name, and save
        ...
        self.wait.until(EC.visibility_of_element_located((By.CLASS_NAME, 'vuetable-body')))
        user_tr = self.driver.find_element_by_xpath("//tr//td[1][contains(text(), '2')]").find_element_by_xpath("..")
        self.assertTrue(user_tr.find_element_by_xpath(".//td[contains(text(), " + last_name + ")]"))


if __name__ == "__main__":
    import __main__
    output = run_test(TestUserLastNameEdited, data, __main__)
