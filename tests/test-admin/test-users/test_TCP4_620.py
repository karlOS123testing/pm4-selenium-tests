#!/usr/local/bin/python3
""" Class to edit a user's password.
"""

# Check if using local environment
from os import getenv

if getenv('ENVIRONMENT') != 'local':
    from test_parent import BaseTest
    from util import run_test, login
# If using local environment
else:
    from sys import path
    path.append('../')
    from includes.test_parent import BaseTest
    from includes.util import run_test, login
    from __init__ import data

import unittest
import string
import random
from selenium.webdriver.support.ui import WebDriverWait, Select
from selenium.webdriver.common.by import By
from selenium.webdriver.support import expected_conditions as EC

class TestUserPasswordRegionEdited(BaseTest):
    ''' Navigate to the Users page and edit a user's password. '''

    def test_that_user_password_is_edited(self):
        ''' Verify that a user's password is successfully edited.'''
        
        # Login using configured url, workspace, username, and password
        self.driver = login(data['server_url'], data['username'], data['password'], self.driver)

        # Redirect to Admin Users page, wait for +User button to be clickable 
        self.driver.get(data['server_url'] + '/admin/users')
        self.wait.until(EC.visibility_of_element_located((By.CLASS_NAME, 'vuetable-body')))
        
        # Find table record with ID = '2', find edit button in this element, and click
        user_tr = self.driver.find_element_by_xpath("//tr//td[1][contains(text(), '2')]").find_element_by_xpath("..")
        user_tr.find_element_by_class_name('fa-pen-square').click()
        
        # Wait for user edit form to load, change password, grab username, and save
        self.wait.until(EC.visibility_of_element_located((By.ID, 'password')))
        username = self.driver.find_element_by_id('username').get_property('value')
        password = ''.join(random.choice(string.ascii_letters) for n in range(10))
        self.driver.find_element_by_id('password').send_keys(password)
        self.driver.find_element_by_id('confPassword').send_keys(password)
        self.driver.find_element_by_id('saveUser').click()

        # Log out and wait for login page to load
        self.wait.until(EC.visibility_of_element_located((By.CLASS_NAME, 'avatar-image-button')))
        self.driver.find_element_by_class_name('avatar-image-button').click()
        self.wait.until(EC.visibility_of_element_located((By.LINK_TEXT, 'Log Out')))
        self.driver.find_element_by_link_text('Log Out').click()
        self.wait.until(EC.visibility_of_element_located((By.CLASS_NAME, 'form-group')))

        # Log in and verify Requests page displays
        #self.driver.find_element_by_id('username').click()
        #self.driver.find_element_by_id('username').send_keys(username)
        #self.wait.until(lambda driver: self.driver.execute_script('return jQuery.active') == 0)
        #self.wait.until(lambda driver: self.driver.execute_script('return document.readyState') == 'complete')
        self.driver.find_element_by_id('username').send_keys(username)
        self.driver.find_element_by_id('password').send_keys(password)
        self.driver.find_element_by_xpath("//button[@name='login']").click()
        self.wait.until(EC.visibility_of_element_located((By.ID, 'navbar-request-button')))
        self.assertTrue(self.driver.find_element_by_id('navbar-request-button'))

if __name__ == "__main__":
    import __main__
    output = run_test(TestUserPasswordRegionEdited, data, __main__)
