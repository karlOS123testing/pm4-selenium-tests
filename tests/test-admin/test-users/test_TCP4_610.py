...
class TestUserEmailEdited(BaseTest):
    ''' Navigate to the Users page and edit a user's email. '''

    def test_that_user_email_is_edited(self):
        ''' Verify that a user's email is successfully edited.'''
        ...
        # Wait for user edit form to load, change email, and save
        ...
        # Wait for edit user form to load and verify email field contains correct email
        self.wait.until(EC.visibility_of_element_located((By.ID, 'email')))
        email_field = self.driver.find_element_by_id('email')
        self.assertEqual(email_field.get_property('value'), email)


if __name__ == "__main__":
    import __main__
    output = run_test(TestUserEmailEdited, data, __main__)
