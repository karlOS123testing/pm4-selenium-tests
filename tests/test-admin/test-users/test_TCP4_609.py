...
""" Class to edit a user's job title.
"""
...
class TestUserJobTitleEdited(BaseTest):
    ''' Navigate to the Users page and edit a user's job title. '''

    def test_that_user_job_title_is_edited(self):
        ''' Verify that a user's job title is successfully edited.'''
        ...
        # Wait for user edit form to load, change job title, and save
        ...
        # Wait for edit user form to load and verify job title field contains correct job title
        self.wait.until(EC.visibility_of_element_located((By.ID, 'title')))
        job_title_field = self.driver.find_element_by_id('title')
        self.assertEqual(job_title_field.get_property('value'), job_title)


if __name__ == "__main__":
    import __main__
    output = run_test(TestUserJobTitleEdited, data, __main__)
