...
""" Class to edit a user's username.
"""
...
class TestUserUsernameEdited(BaseTest):
    ''' Navigate to the Users page and edit a user's username. '''

    def test_that_user_username_is_edited(self):
        ''' Verify that a user's username is successfully edited.'''
        ...
        # Wait for user edit form to load, change username, and save
        ...
        # Navigate to Users page and wait for Users table to load and verify that new username exists for table record 2
        self.driver.get(data['server_url'] + '/admin/users')
        self.wait.until(EC.visibility_of_element_located((By.CLASS_NAME, 'vuetable-body')))
        user_tr = self.driver.find_element_by_xpath("//tr//td[1][contains(text(), '2')]").find_element_by_xpath("..")
        self.assertTrue(user_tr.find_element_by_xpath(".//td[contains(text(), " + username + ")]"))


if __name__ == "__main__":
    import __main__
    output = run_test(TestUserUsernameEdited, data, __main__)
