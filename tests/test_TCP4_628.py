...
class TCP4_628(BaseTest):
    ''' Test that the status of an user's admin group'''

    def test_tcp4_628(self):
        '''Edits an users admin group status'''
        ...
        if self.driver.find_element_by_css_selector("div>input[id='switch_3']").is_selected():
            unselected = True
        else:
            unselected = False

        # Interact with the process administrator group slider, save and wait for success alert
        # Uses JSexecutor as a normal click wont work
        javaScript = "document.getElementById('switch_3').click();"
        ...
        # Verify process administrator Group togglehas changed
        self.wait.until(EC.visibility_of_element_located((By.ID, "nav-groups")))
        if unselected:
            self.assertFalse(self.driver.find_element_by_css_selector("div>input[id='switch_3']").is_selected())
        else:
            self.assertTrue(self.driver.find_element_by_css_selector("div>input[id='switch_3']").is_selected())


if __name__ == "__main__":
    import __main__     
    output = run_test(TCP4_628, data, __main__)